# Hi there, I'm Wesley Ng 👋

I am a Computer Engineering senior at the Singapore University of Technology and Design (SUTD).
I specialize in bridging hardware and software, with a strong focus on systems architecture, hardware telemetry, and distributed network environments. 

Currently, I am looking for roles in **Network Engineering** and **Data Center Infrastructure**, where I can apply my background in building secure, highly reliable physical and digital systems.

### 🛠️ Technical Stack
* **Systems & OS:** Linux (Ubuntu), SSH, Git, ROS 2 (Distributed Node Networks) 
* **Programming:** Python, C++, Scala 
* **Hardware:** Microcontrollers (MCU), Hardware Telemetry, Fusion 360 

### 🚀 Featured Work

* **[AMR Real-Time Telemetry Sensor](https://github.com/friyk/AMR-Temperature-Sensor)** 
  * Developed a battery temperature telemetry sensor during my time at DSTA to detect thermal runaway anomalies, ensuring hardware safety and operational continuity.
* **Autonomous Luggage Concierge (B.HIVE)** [Weighing and Loading](https://github.com/friyk/2026S23CapstoneBHIVEAMR), [AMR](https://github.com/friyk/2026S23CapstoneBHIVELOADANDWEIGH.git) 
  * Architected a distributed communications network with a Jetson Orin Nano using ROS 2, managing real-time data publishing/subscribing for an industry-sponsored robotics ecosystem.
* **Hardware Security: CSRNG** 
  * Designed a Cryptographically Secure Random Number Generator utilizing environmental radiation. Co-authored and published the hardware architecture and statistical validation findings on [IEEE](https://doi.org/10.1109/TENCON66050.2025.11375162).

### 📬 Let's Connect
* **LinkedIn:** [linkedin.com/in/wesleyngah](https://linkedin.com/in/wesleyngah)
* **Email:** Wesleyngah@gmail.com
* **Resume:** [View my Resume](link_to_resume_pdf_if_hosted)
